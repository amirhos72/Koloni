...
const POSTS = [
  {
    id: 1,
    title: "Discover the Best Events with Koloni: Your Ultimate Guide",
    excerpt: "Navigate the social landscape with AI-driven recommendations tailored to your unique vibe.",
    author: "Armita",
    date: "14 Dec 2025",
  ...
  },
  {
    ...
    excerpt: "How our social mining protocol rewards authentic interactions in real-time.",
    author: "System",
    date: "12 Dec 2025",
  ...
  },
  {
    ...
    excerpt: "Everything you need to know about the invite-only tier reshaping nightlife.",
    author: "Sarah J.",
    date: "08 Dec 2025",
  ...
  },
  {
     ...
     excerpt: "Forecasting capacity before the doors even open. A technical deep dive.",
     author: "Dev Team",
     date: "01 Dec 2025",
  ...
  return (
    <section id="blog" className="py-16 md:py-32 bg-transparent relative border-t border-white/5 overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-6 relative z-10">
            ...
            {/* Filter Buttons */}
            <div className="flex flex-wrap gap-2 md:gap-3 mb-8 md:mb-10 animate-fade-in" style={{ animationDelay: '0.2s' }}>
                {CATEGORIES.map(cat => (
                    <button
                        key={cat}
                        onClick={() => setActiveCategory(cat)}
                        className={`px-4 md:px-6 py-2 md:py-2.5 text-[9px] md:text-[10px] font-bold uppercase tracking-widest border transition-all duration-300 rounded-full ${
                            activeCategory === cat
            ...
            >
                {filteredPosts.map((post, i) => (
                    <article 
                        key={post.id} 
                        className="min-w-[280px] md:min-w-[420px] snap-center md:snap-start group cursor-pointer flex flex-col"
                        style={{ animationDelay: `${i * 0.1}s` }}
                    >
                        {/* Image Container */}
                        <div className="relative aspect-[16/10] overflow-hidden mb-4 md:mb-6 border border-white/10 bg-white/5 rounded-2xl transition-all duration-500 group-hover:border-koloni-gold/40 group-hover:shadow-[0_10px_40px_-10px_rgba(0,0,0,0.5)]">
                            ...
                            <div className="hidden md:block absolute bottom-4 right-4 translate-y-4 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-300">
                        ...
                        <div className="space-y-3 md:space-y-4 pr-2">
                            ...
                            <div className="flex items-center justify-between w-full">
                                <span className="text-[9px] font-bold bg-white/5 text-koloni-gold px-3 py-1 uppercase tracking-widest border border-white/10 rounded-full backdrop-blur-sm">
                                    {post.category}
                                </span>
                                <div className="flex items-center gap-2 text-[10px] md:text-xs font-mono text-gray-400 group-hover:text-white transition-colors">
                                    <User size={12} className="text-koloni-gold" />
                                    <span>{post.author}</span>
                                </div>
                            </div>
                            
                            <h3 className="font-display font-bold text-lg md:text-2xl text-white group-hover:text-koloni-gold transition-colors leading-tight min-h-[3rem] md:min-h-[3.5rem]">
                                {post.title}
                            </h3>
